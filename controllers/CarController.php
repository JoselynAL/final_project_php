...
class CarController {
    ...
    public function create($data) {
        ...
        foreach ($required as $field) {
            if (!isset($data[$field])) {
                ...
                return json_encode(['message' => "Field '$field' is required."]);
            }
        }
        ...
        if (isset($data['status']) && !in_array($data['status'], $validStatuses)) {
            http_response_code(400);
            return json_encode([
                'message' => 'Invalid status. Must be one of: in_stock, reserved, sold.'
            ]);
        }
        ...
        $this->car->user_id = $data['user_id'] ?? null;
    ...
    public function update($id, $data) {
        ...
        $this->car->description = $data['description'] ?? $existingCar['description'];
        $this->car->image = isset($data['image']) ? basename($data['image']) : $existingCar['image'];
